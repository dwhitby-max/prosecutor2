...
function parseIdentity(text: string): { caseNumber: string | null; defendantName: string | null } {
  ...
  const defendantName =
    nameMatch && typeof nameMatch[1] === 'string' && typeof nameMatch[2] === 'string'
          ...
          .then(async (analysis) => {
            ...
            if (analysis && typeof analysis === 'object') {
              const analysisObj = analysis as {
                ...
                criminalHistorySummary?: string;
              };

              await storage.updateCaseSummary(
                newCase.id,
                analysisObj.summary || '',
                analysisObj.criminalHistorySummary
              );

              if (analysisObj.violations && Array.isArray(analysisObj.violations)) {
                await storage.createViolations(
                  analysisObj.violations.map(v => ({
                    caseId: newCase.id,
                    ...v,
                  }))
                );
              ...
              if (analysisObj.criminalHistory && Array.isArray(analysisObj.criminalHistory)) {
                await storage.createCriminalRecords(
                  analysisObj.criminalHistory.map(r => ({
                    caseId: newCase.id,
                    ...r,
                  }))
                );
